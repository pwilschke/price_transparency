...
# automatically clear anything in the temp directory when running
...
parse_json <- function(path, prov) {
  root <- tryCatch(
    RcppSimdJson::fload(path, max_simplify_lvl = "list"),  # much faster read than jsonlite for big files
    error = function(e) jsonlite::fromJSON(path, simplifyVector = FALSE)
  )
  ...
  # ---- pass 1: count exactly how many output rows we'll need, so we can
  # preallocate once instead of growing the list element-by-element (that
  # growth pattern is O(n^2) and is almost certainly the real slowdown here).
  n_total <- 0L
  for (item in sci) {
  ...
  for (item in sci) {
    ...
    for (sc in ga(item, "standard_charges")) {
      ...
      base <- list(
        description = desc,
        primary_code = if (length(codes)) codes[1] else NA_character_,
        primary_code_type = if (length(ctypes)) ctypes[1] else NA_character_,
        setting = chr1(sc[["setting"]]),
        drug_unit_of_measurement = d_unit, drug_type_of_measurement = d_type,
        standard_charge_gross = gross$value, standard_charge_discounted_cash = cash$value,
        standard_charge_min = mn$value, standard_charge_max = mx$value,
        additional_generic_notes = chr1(sc[["additional_generic_notes"]]),
        codes = list(codes), code_types = list(ctypes),
        modifiers = list(as.character(unlist(ga(sc, "modifier_code"))))
      )
      ...
      if (length(payers) == 0) {
        k <- k + 1L
        rows[[k]] <- c(base, list(payer_name = NA_character_, plan_name = NA_character_, parse_ok = item_ok))
        next
      }
      
      # vectorized across all payers for this standard_charge entry, instead
      # of looping payer-by-payer and re-doing field lookups each time
      payer_name  <- vapply(payers, function(p) chr1(p[["payer_name"]]), character(1))
      plan_name   <- vapply(payers, function(p) chr1(p[["plan_name"]]), character(1))
      neg_algo    <- vapply(payers, function(p) chr1(p[["standard_charge_algorithm"]]), character(1))
      methodology <- vapply(payers, function(p) chr1(p[["methodology"]]), character(1))
      cnt         <- vapply(payers, function(p) chr1(p[["count"]]), character(1))
      pay_notes   <- vapply(payers, function(p) chr1(p[["additional_payer_notes"]]), character(1))
      
      nd_list  <- lapply(payers, function(p) num(p[["standard_charge_dollar"]]))
      npc_list <- lapply(payers, function(p) num(p[["standard_charge_percentage"]]))
      med_list <- lapply(payers, function(p) num(p[["median_amount"]]))
      p10_list <- lapply(payers, function(p) num(p[["10th_percentile"]]))
      p90_list <- lapply(payers, function(p) num(p[["90th_percentile"]]))
      
      for (j in seq_along(payers)) {
        k <- k + 1L
        rows[[k]] <- c(base, list(
          payer_name = payer_name[j], plan_name = plan_name[j],
          negotiated_dollar = nd_list[[j]]$value, negotiated_percentage = npc_list[[j]]$value,
          negotiated_algorithm = neg_algo[j], methodology = methodology[j],
          median_amount = med_list[[j]]$value, p10_percentile = p10_list[[j]]$value,
          p90_percentile = p90_list[[j]]$value,
          count = cnt[j], additional_payer_notes = pay_notes[j],
          parse_ok = item_ok && nd_list[[j]]$ok && npc_list[[j]]$ok
        ))
      }
    }
  }
  
  out <- if (k > 0) rbindlist(rows[seq_len(k)], fill = TRUE) else data.table()
  out <- attach_meta(out, meta, prov)
  ensure_schema(out)
}
# ---- process_blob ---------------------------------------------------------
...
for (i in seq_len(nrow(todo))) {
  ...
  if (i %% CHECKPOINT_EVERY == 0) {
    log_msg("...%d / %d blobs processed", i, nrow(todo))
    flush_progress(all_logs, newly_done)
    all_logs <- list(); newly_done <- character(0)   # already flushed; don't rewrite
  }
}
# flush whatever remains since the last checkpoint (a partial final chunk)
flush_progress(all_logs, newly_done)
